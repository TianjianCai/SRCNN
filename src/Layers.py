...
class none_linear_mapping(object):
    def __init__(self,x,kernel_size=1,strides=1,channels_in=3,channels_out=3,W=None,B=None,keep_prob=1):
        if W is None:
            #self.W = tf.Variable(tf.random_uniform(shape=[kernel_size,kernel_size,channels,channels],minval=0,maxval=1))
            self.W = tf.Variable(tf.random_normal(shape=[kernel_size,kernel_size,channels_in,channels_out]))
        else:
...
class reconstruction(object):
    def __init__(self,x,kernel_size=7,strides=1,channels_in=3,channels_out=3,W=None,B=None,keep_prob=1):
        if W is None:
            #self.W = tf.Variable(tf.random_uniform(shape=[kernel_size,kernel_size,channels,channels],minval=0,maxval=1))
            self.W = tf.Variable(tf.random_normal(shape=[kernel_size,kernel_size,channels_in,channels_out]))
        else:
